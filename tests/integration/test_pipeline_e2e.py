...
ACCOUNT_IDS = ["corsino-marketing-labs"]
DATE = "2026-04-11"

with open("config/lead_classification_rules.json") as _f:
    _RULES = json.load(_f)
...
def _build_pipeline(p):
    """Wire all transforms onto pipeline p. Returns (all_records, crm_dead_letter, crm_enriched)."""
    rules = p | "Rules" >> beam.Create([_RULES])

    analytics = build_analytics(p, bucket=BUCKET, account_ids=ACCOUNT_IDS, date=DATE)
    crm_valid, crm_dead_letter = build_crm(p, bucket=BUCKET)
    ...
    return all_records, crm_dead_letter, crm_enriched
...
def test_dead_letter_count():
    ...
    with TestPipeline() as p:
        _, crm_dead_letter, _ = _build_pipeline(p)
        assert_that(_count(crm_dead_letter, "DL"), equal_to([EXPECTED_DEAD_LETTER]))
...
def test_dead_letter_reasons():
    ...
    with TestPipeline() as p:
        ...
        missing = crm_dead_letter | "FilterMissing" >> beam.Filter(
            lambda r: r["reason"] == "missing_analytics_user_id"
        )
        invalid = crm_dead_letter | "FilterInvalid" >> beam.Filter(
...
def test_dead_letter_schema():
    ...
    with TestPipeline() as p:
        ...
        malformed = crm_dead_letter | beam.Filter(
...
def test_beam_metrics():
    ...
    p = TestPipeline()
    all_records, crm_dead_letter, crm_enriched = _build_pipeline(p)
    ...
    assert_that(
        _count(crm_dead_letter, "DLM"), equal_to([EXPECTED_DEAD_LETTER]), label="dl_m"
    )
    assert_that(
